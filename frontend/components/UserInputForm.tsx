...
export function UserInputForm({
  ...
  const isFormValid =
    interests.trim().length >= 10 &&
    aptitudes.trim().length >= 10 &&
    pursuitValues.trim().length >= 10 &&
    candidateMajors.length >= 3;

  const toggleMajor = (major: string) => {
    if (candidateMajors.includes(major)) {
      onCandidateMajorsChange(candidateMajors.filter((m) => m !== major));
    } else {
  ...
  return (
    <div className="flex flex-col gap-6 flex-1 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2">
                  {majors.map((major) => (
                    <button
                      ...
                      disabled={isGenerating}
                      className={`px-3 py-2 rounded-lg border transition-colors text-sm ${candidateMajors.includes(major)
      ...
      <Button
        ...
        className="w-full h-14 text-base bg-[#FF1F55] hover:bg-[#FF4572] text-white border-0"
        size="lg"
      >
        {isGenerating ? (
          <>
            <span className="inline-block size-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2" />
            에이전트 생성 중...
          </>
        ) : (
          <>
