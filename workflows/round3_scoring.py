...
각 핵심 평가(2-3개)에 대해 반드시 다음 구조를 따르세요:
...
"근거: 
- 사용자가 '높은 연봉'을 명시했고, 컴공은 신입 평균 초봉 5,500만원으로 5개 전공 중 최고
- '빠른 성장'을 원하는 사용자에게 5년차 평균 8,200만원(49% 상승)으로 성장률도 최고
- 사용자의 '코딩 능력 우수'는 고연봉 직무(백엔드, AI)와 직결되므로 8.5점"

"데이터사이언스 - 워라밸: 5.5"
"근거:
- 사용자가 '지속 가능한 커리어'를 언급했으나, 데이터 직무는 마감 압박이 큼
- 프로젝트 기반 업무로 주 50-55시간 근무가 일반적
...
❌ 나쁜 반박:
"산업디자인 - 학문적 깊이: 4.5는 과소평가입니다"

✅ 좋은 반박:
"**산업디자인 - 학문적 깊이와 연구 기회**
- 제안된 점수: 4.5
- 문제점: 사용자가 '체계적 사고'를 언급했는데, 산업디자인의 다학제 연구(인간공학, 재료공학, UX 연구)를 간과. 실제 디자인 대학원 연구실 수는 컴공의 70% 수준
- 적절한 점수: 6.5
- 근거: 사용자의 '논리적 분석' 강점 + 디자인씽킹 방법론의 체계성 + 석사 진학률 35% = 6.5점"

[중요 지침]
1. 사용자의 흥미/적성/가치에서 구체적 키워드를 인용하세요.
2. 전공의 실제 특성(커리큘럼, 산업 현황, 통계)을 언급하세요.
3. 점수 차이에 대한 명확한 논리를 제시하세요.

---

**주의사항:**
- 200-250자로 논리적으로 반박
- 반드시 사용자의 구체적 키워드를 인용
- 전공의 객관적 특성이나 통계 제시
- 점수 산정 논리 명확히 설명
"""
