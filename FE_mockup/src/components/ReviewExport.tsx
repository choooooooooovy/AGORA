...
import { Award, BarChart3, Target, Sparkles } from "lucide-react";
...
export function ReviewExport({ recommendations, candidateMajors, agents }: ReviewExportProps) {
  const getConfidenceLevel = (coefficient: number): string => {
    if (coefficient >= 0.8) return "매우 높음";
    if (coefficient >= 0.7) return "높음";
    if (coefficient >= 0.6) return "중간";
    if (coefficient >= 0.5) return "보통";
    return "낮음";
  };

  const getConfidenceColor = (coefficient: number): string => {
    if (coefficient >= 0.8) return "text-green-400";
    if (coefficient >= 0.7) return "text-[#FF4572]";
    if (coefficient >= 0.6) return "text-yellow-400";
    if (coefficient >= 0.5) return "text-orange-400";
    return "text-gray-400";
  };

  // Mock data for detailed analysis
  const topMajors = candidateMajors.length > 0 
    ? candidateMajors.slice(0, 3)
    : ["컴퓨터공학", "데이터사이언스", "철학"];
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[#9ca6ba] mt-2">
              TOPSIS 분석을 통해 도출된 {topMajors.length}개 전공의 최종 순위
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {topMajors.map((major, index) => {
            ...
            const rank = index + 1;
            const topsisScore = rank === 1 ? 0.8547 : rank === 2 ? 0.7821 : 0.7354;
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  {/* Rank Badge */}
                  <div
                    className={`flex items-center justify-center size-16 rounded-full flex-shrink-0 ${
                      rank === 1
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-white text-xl">{major}</h3>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-[#9ca6ba] text-sm">TOPSIS 점수:</span>
                          <span className="text-[#FF1F55]">{topsisScore.toFixed(4)}</span>
                        </div>
                      </div>
                      {rank === 1 && (
      ...
      <div className="w-96 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="bg-black/20 border border-[#282e39] rounded-lg p-4">
          <h3 className="text-white mb-4 flex items-center gap-2">
            <BarChart3 className="size-5 text-[#FF1F55]" />
            기준별 가중치
          </h3>
